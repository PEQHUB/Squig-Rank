/**
 * ISO 226:2003 loudness normalization
 * Faithfully ported from squig.link/CrinGraph graphtool.js
 * Source: https://github.com/squiglink/lab/blob/main/graphtool.js
 *
 * This module provides findLoudnessOffset(), which computes the dB offset
 * needed to make a frequency response curve's perceived loudness equal to
 * a target in phon. This is used for level-matching IEM and target curves
 * before PPI scoring, replacing the old single-frequency (1 kHz) method.
 *
 * Unlike single-frequency normalization (e.g. at 1 kHz), this method is
 * robust against localized dips or peaks because it uses the entire curve
 * weighted by how the human ear perceives loudness at each frequency.
 */
...
const ISO223_PARAMS = {
  // Standard ISO 226:2003 reference frequencies
  f: [
  ...
  // Upper limit of hearing (normal hearing threshold offset)
  L_U: [
...
// Free-field correction values at 1/48-octave resolution (480 entries)
// Each value is offset by -7 dB from the raw free-field response.
// Source: CrinGraph/squig.link graphtool.js
 ...
 */
function initNormalize(freqs: number[]): NormParams[] {
  ...
  for (const f of freqs) {
    ...
    let a: number, m: number, k: number;
    if (i0 === i1) {
      a = p.a_f[i0];
...
/** Default target loudness in phon (matches CrinGraph's default of 60 dB) */
export const DEFAULT_NORM_PHON = 60;

/**
 * Compute the loudness-weighted average dB of a curve.
 * Returns a single dB value representing the perceptual center of the curve,
 * weighted by ISO 226:2003 ear sensitivity at each frequency.
 * This is robust against localized dips/peaks unlike a single-frequency reference.
 */
export function loudnessWeightedCenter(
  frequencies: number[],
  db: number[]
): number {
  ...
  for (let i = 0; i < params.length; i++) {
    ...
    // Compute the loudness sensitivity weight at this frequency
    // This is the derivative of the loudness function w.r.t. dB at this point
...
/**
 * Find the dB offset that, when ADDED to each dB value in the curve,
 * makes the total perceived loudness equal to the target in phon.
 *
 * Faithfully reproduces find_offset() from graphtool.js.
 * Uses Newton's method with an initial guess based on average dB level.
 *
 * @param frequencies - Array of frequency values in Hz
 * @param db - Array of dB values (same length as frequencies)
 * @param targetPhon - Target loudness in phon (default 60)
 * @returns The dB offset to ADD to each dB value
 */
export function findLoudnessOffset(
  frequencies: number[],
  db: number[],
  targetPhon: number = DEFAULT_NORM_PHON
): number {
  const params = initNormalize(frequencies);
  const l10 = Math.log(10) / 10;
  // Use average dB as initial guess for robust convergence
  const avgDb = db.reduce((a, b) => a + b, 0) / db.length;
  let x = avgDb - targetPhon;

  function getStep(o: number): number {
    let v = 0;
    let d = 0;
